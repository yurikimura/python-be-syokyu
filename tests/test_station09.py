...
def test_get_todo_item(db_session) -> None:
    ...
    # ******************
    # テスト用にTODOリストをインサート
    db_todo_list = list_model.ListModel(title="station09_test", description="A test record for station09.")
    ...
    db_todo_item = item_model.ItemModel(
        ...
        title="station09_test",
        description="A test record for station09.",
        status_code=1,
    )
    ...
    assert response_body["title"] == "station09_test"
    assert response_body["description"] == "A test record for station09."
